# Ghost Wick v5.1 — PRE-CATALYST DETECTION ◎ SUPERIOR
...
indicator("GHOST WICK v5.1 ◎ SUPERIOR", overlay=true, max_lines_count=500, max_boxes_count=500, max_labels_count=500)
...
if barstate.islast
    ...
    table.cell(d, 0, 0, "GHOST WICK v5.1 ◎", text_color=color.white, text_size=size.normal, bgcolor=color.new(color.black,35))
...
alertcondition(enter_loaded, "◎ LOADED", "v5.1: loaded at liquidity level")
alertcondition(enter_stalk, "⊙ STALKING", "v5.1: stalking — pre-HTF loaded at reversal zone")
alertcondition(enter_long, "◉ LONG", "v5.1: trend long entry")
alertcondition(enter_short, "◉ SHORT", "v5.1: trend short entry")
alertcondition(enter_range_long, "◉ FADE LONG", "v5.1: range fade long")
alertcondition(enter_range_short,"◉ FADE SHORT", "v5.1: range fade short")
alertcondition(enter_cont_long, "◉ CONT LONG", "v5.1: continuation long")
alertcondition(enter_cont_short, "◉ CONT SHORT", "v5.1: continuation short")
alertcondition(enter_disp_long, "◉ DISP LONG", "v5.1: displacement breakout long — trend + RVOL + CVD + body dominance")
alertcondition(enter_disp_short, "◉ DISP SHORT", "v5.1: displacement breakout short — trend + RVOL + CVD + body dominance")
alertcondition(enter_abs_long, "◉ ABSORB LONG", "v5.1: absorption breakout/spring long")
alertcondition(enter_abs_short, "◉ ABSORB SHORT", "v5.1: absorption breakout/upthrust short")
alertcondition(move_to_manage, "◈ TP1 HIT", "v5.1: partial TP, trailing")
alertcondition(obv_exit_fired, "◈ OBV EXIT", "v5.1: OBV flow exit — distribution detected")
alertcondition(rev_exit_fired, "◈ REV EXIT", "v5.1: Reversal exit — REV + RSI divergence against trade while in profit")
alertcondition(retest_reentry, "◉ RE-ENTRY", "v5.1: retest re-entry at structural zone")
alertcondition(exit_win, "✓ WIN", "v5.1: trade closed in profit")
alertcondition(exit_loss, "✗ EXIT", "v5.1: stopped or invalidated")
alertcondition(bb_squeeze and not bb_squeeze[1], "◆ BB SQUEEZE START", "v5.1: Bollinger Band squeeze active — compression phase detected")
alertcondition(bb_expanding and not bb_expanding[1] and bb_squeeze_ctx, "◆ BB SQUEEZE BREAKOUT", "v5.1: BB squeeze expanding — breakout imminent")
alertcondition(rev_bull, "◆ REV BULL", "v5.1: Momentum reversal bull — sweep + RVOL at sellside liquidity")
alertcondition(rev_bear, "◆ REV BEAR", "v5.1: Momentum reversal bear — sweep + RVOL at buyside liquidity")
alertcondition(dist_bull, "◆ ACCUM BULL", "v5.1: Quiet accumulation — RSI div + declining vol at sellside (no sweep)")
alertcondition(dist_bear, "◆ DIST BEAR", "v5.1: Quiet distribution — RSI div + declining vol at buyside (no sweep)")
alertcondition(rsi_hid_bull_div, "hRSI BULL", "v5.1: Hidden bullish RSI divergence — continuation signal in uptrend")
alertcondition(rsi_hid_bear_div, "hRSI BEAR", "v5.1: Hidden bearish RSI divergence — continuation signal in downtrend")
alertcondition(wyckoff_phase_c and not wyckoff_phase_c[1], "WYCKOFF C — SPRING", "v5.1: Wyckoff Phase C spring detected — high-conviction absorption entry zone")
alertcondition(wyckoff_phase_d and not wyckoff_phase_d[1], "WYCKOFF D — BOS", "v5.1: Wyckoff Phase D BOS — accumulation confirming, markup beginning")
alertcondition(wyckoff_phase_e and not wyckoff_phase_e[1], "WYCKOFF E — MARKUP", "v5.1: Wyckoff Phase E markup — price has left the range on volume")
alertcondition(demand_zone_fresh and not demand_zone_fresh[1], "DEMAND ZONE FRESH TOUCH", "v5.1: Fresh demand zone first touch — highest-quality institutional level")
alertcondition(supply_zone_fresh and not supply_zone_fresh[1], "SUPPLY ZONE FRESH TOUCH", "v5.1: Fresh supply zone first touch — highest-quality institutional level")
alertcondition(range_confirmed and not range_confirmed[1], "Range confirmed", "v5.1: switching to fade playbook")
alertcondition(trend_confirmed and not trend_confirmed[1], "Trend confirmed", "v5.1: switching to displacement playbook")
alertcondition(abs_no_edge and not abs_no_edge[1], "ABS NO EDGE", "v5.1: absorption mode killed — no edge after evaluation")
```
