# Ghost Wick v4.5 — PRE-CATALYST DETECTION ◎ SUPERIOR
...
// ═══════════════════════════════════════════════════════════
// PRE-CATALYST DETECTION v4.5 ◎ SUPERIOR
//
// v4.5 CHANGES:
...
// Wick rejection + RVOL + near_sellside fires frequently in volatile
// drops — EMA slope gate correctly filtered these as non-reversals.
// PENGU: 62W/26L +8.5R (v4.4) → 99W/101L -45.4R (v4.5 broken).
// Reverted to v4.4 behavior. FIX-17 needs redesign: require
// additional confirmation (e.g. CVD lean + RSI div confluence)
// before bypassing EMA slope, not blanket bypass.
//
// v4.4 CHANGES:
...
indicator("GHOST WICK v4.5 ◎ SUPERIOR", overlay=true, max_lines_count=500, max_boxes_count=500, max_labels_count=500)
...
i_trend_prob = input.float(0.45,"Prob threshold (trend floor) (FIX-11)", minval=0.25, maxval=0.7, step=0.05, group=grp_state, tooltip="v4.5: ADX-interpolated threshold floor for trend regime.\nRange (ADX≤18): uses ceiling (0.55). Trend (ADX≥25): uses this floor (0.45).\nContinuously interpolates between. Matches FIX-4 weight matrix scaling.")
...
i_adx_range = input.float(18.0,"ADX range threshold (FIX-2)", minval=12, maxval=28, step=1.0, group=grp_guard, tooltip="v4.5: Widened from 20-18. ADX hysteresis prevents dead-zone oscillation.")
i_adx_trend = input.float(25.0,"ADX trend threshold (FIX-2)", minval=18, maxval=38, step=1.0, group=grp_guard, tooltip="v4.5: Widened from 22-25. 7-point gap with hysteresis eliminates false regime flips.")
...
if trade_state == -1 and bar_confirmed and not cooldown_active
...
if trade_state == -1 and bar_confirmed and near_buyside and bear_prob >= i_range_prob and bear_prob > bull_prob and conviction_ok
    trade_state := 2
    entry_bar_idx := bar_index
    trade_dir := -1
    is_range_trade := true
...
if barstate.islast
    ...
    table.cell(d, 0, 0, "GHOST WICK v4.5 ◎", text_color=color.white, text_size=size.normal, bgcolor=color.new(color.black,35))
...
alertcondition(enter_loaded, "◎ LOADED", "v4.5: loaded at liquidity level")
alertcondition(enter_stalk, "⊙ STALKING", "v4.5: stalking — pre-HTF loaded at reversal zone")
alertcondition(enter_long, "◉ LONG", "v4.5: trend long entry")
alertcondition(enter_short, "◉ SHORT", "v4.5: trend short entry")
alertcondition(enter_range_long, "◉ FADE LONG", "v4.5: range fade long")
alertcondition(enter_range_short,"◉ FADE SHORT", "v4.5: range fade short")
alertcondition(enter_cont_long, "◉ CONT LONG", "v4.5: continuation long")
alertcondition(enter_cont_short, "◉ CONT SHORT", "v4.5: continuation short")
alertcondition(enter_abs_long, "◉ ABSORB LONG", "v4.5: absorption breakout/spring long")
alertcondition(enter_abs_short, "◉ ABSORB SHORT", "v4.5: absorption breakout/upthrust short")
alertcondition(move_to_manage, "◈ TP1 HIT", "v4.5: partial TP, trailing")
alertcondition(obv_exit_fired, "◈ OBV EXIT", "v4.5: OBV flow exit — distribution detected")
alertcondition(retest_reentry, "◉ RE-ENTRY", "v4.5: retest re-entry at structural zone")
alertcondition(exit_win, "✓ WIN", "v4.5: trade closed in profit")
alertcondition(exit_loss, "✗ EXIT", "v4.5: stopped or invalidated")
alertcondition(bb_squeeze and not bb_squeeze[1], "◆ BB SQUEEZE START", "v4.5: Bollinger Band squeeze active — compression phase detected")
alertcondition(bb_expanding and not bb_expanding[1] and bb_squeeze_ctx, "◆ BB SQUEEZE BREAKOUT", "v4.5: BB squeeze expanding — breakout imminent")
alertcondition(rev_bull, "◆ REV BULL", "v4.5: Momentum reversal bull — sweep + RVOL at sellside liquidity")
alertcondition(rev_bear, "◆ REV BEAR", "v4.5: Momentum reversal bear — sweep + RVOL at buyside liquidity")
alertcondition(rsi_reg_bull_div, "RSI DIV BULL", "v4.5: Regular bullish RSI divergence — potential reversal")
alertcondition(rsi_hid_bull_div, "hRSI BULL", "v4.5: Hidden bullish RSI divergence — continuation signal in uptrend")
alertcondition(rsi_reg_bear_div, "RSI DIV BEAR", "v4.5: Regular bearish RSI divergence — potential reversal")
alertcondition(rsi_hid_bear_div, "hRSI BEAR", "v4.5: Hidden bearish RSI divergence — continuation signal in downtrend")
alertcondition(wyckoff_phase_c and not wyckoff_phase_c[1], "WYCKOFF C — SPRING", "v4.5: Wyckoff Phase C spring detected — high-conviction absorption entry zone")
alertcondition(wyckoff_phase_d and not wyckoff_phase_d[1], "WYCKOFF D — BOS", "v4.5: Wyckoff Phase D BOS — accumulation confirming, markup beginning")
alertcondition(wyckoff_phase_e and not wyckoff_phase_e[1], "WYCKOFF E — MARKUP", "v4.5: Wyckoff Phase E markup — price has left the range on volume")
alertcondition(demand_zone_fresh and not demand_zone_fresh[1], "DEMAND ZONE FRESH TOUCH", "v4.5: Fresh demand zone first touch — highest-quality institutional level")
alertcondition(supply_zone_fresh and not supply_zone_fresh[1], "SUPPLY ZONE FRESH TOUCH", "v4.5: Fresh supply zone first touch — highest-quality institutional level")
alertcondition(range_confirmed and not range_confirmed[1], "Range confirmed", "v4.5: switching to fade playbook")
alertcondition(trend_confirmed and not trend_confirmed[1], "Trend confirmed", "v4.5: switching to displacement playbook")
alertcondition(abs_no_edge and not abs_no_edge[1], "ABS NO EDGE", "v4.5: absorption mode killed — no edge after evaluation")
alertcondition(bb_squeeze and absorption_mode and abs_valid_range, "ABSORB + SQUEEZE", "v4.5: Absorption mode active with BB squeeze — Phase B compression confirmed")
```
